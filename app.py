...
# source amadeo_env/bin/activate
...
# Load models
nlp = spacy.load("/Users/amirejibiilia/Desktop/AmadeoChat/amadeo_env/lib/python3.10/site-packages/en_core_web_sm/en_core_web_sm-3.7.1")
...
# Load your data from a local path (remove file upload and use this)
df = pd.read_excel('/Users/amirejibiilia/Desktop/AmadeoChat/amadeo_chatbot.xlsx')
        ...
        if value is not None:
            if find_change and compare_year:
                ...
                if value_compare is not None:
                    ...
                    response = f"{metric_found_message} The {metric.upper()} {'increased' if difference > 0 else 'decreased'} by {abs(difference):.2f} from {year} to {compare_year}."
                    return response
            else:
